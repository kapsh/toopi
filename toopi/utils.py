...
def strict_http_session() -> requests.Session:
    ...
    session.hooks = {"response": lambda r, *a, **kw: r.raise_for_status()}
    return session
...
def tabulate(
    rows: List[Iterable[str]], *, gap=2, titles: List[str] = None
    ...
    col_widths = [max(map(len, column)) + gap for column in zip(*data)]

    for row in data:
        yield "".join(
            field.ljust(width) for field, width in zip(row, col_widths)
        ).rstrip()
