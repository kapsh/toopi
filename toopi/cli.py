...
def parse_args() -> argparse.Namespace:
    ...
    opts.add_argument(
        "-s",
        dest="service",
        choices=paste.SERVICES,
        default=paste.DEFAULT_SERVICE,
        help="Set service to use. Default: %(default)s",
        metavar="SERVICE",
    )
    opts.add_argument(
        "-l",
    ...
    opts.add_argument(
        "-t", dest="title", default="", help="Paste title (where available)"
    )
    ...
    source.add_argument(
        "-x", dest="from_clipboard", action="store_true", help="Paste clipboard content"
    )
    ...
    url.add_argument(
        "-X", dest="to_clipboard", action="store_true", help="Copy URL to clipboard"
    )

    query = parser.add_argument_group("Query options").add_mutually_exclusive_group()
    query.add_argument(
        "-S", dest="list_services", action="store_true", help="List known services"
    )
    query.add_argument(
        "-L",
        dest="list_languages",
    ...
    query.add_argument(
        "-V", dest="version", action="store_true", help="Show program version"
    )
